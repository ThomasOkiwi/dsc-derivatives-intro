...
For this formula, we need to find (XTX), and invert it as well, which makes it very expensive. Imagine the matrix X(N×M+1) has (M+1) columns where M is the number of predictors and N is the number of rows of observations. In machine learning, you will often find datasets with M>1000 and N>1,000,000. The (XTX) matrix itself takes a while to calculate, then you have to invert an M×M matrix which adds more to the complexity - making it very expensive. You'll also come across situations where the input matrix grows so large that it cannot fit into your computer's memory.
## The Big O notation
...
![image](https://github.com/user-attachments/assets/ff5b8020-bb66-41b4-b177-005678868890)

 
## OLS and Big O notation
Inverting a matrix costs O(n3) for computation where n is the number of rows in X matrix, i.e., the observations. Here is an explanation of how to calculate Big O for OLS.
OLS linear regression is computed as (XTX)−1XTy.
If X is an (n×k) matrix:
•	(XTX) takes O(n∗k2) time and produces a (k×k) matrix
•	The matrix inversion of a (k x k) matrix takes O(k3) time
•	(XTY) takes O(n∗k2) time and produces a (k×k) matrix
•	The final matrix multiplication of two (k×k) matrices takes O(k3) time
** So the Big O running time for OLS is O(k2∗(n+k)) - which is pretty expensive **
...
## Gradient Descent
![image](https://github.com/user-attachments/assets/b27f0444-f068-49ca-bbe6-1c1fdc69420d)
...
In regression, it is used to find the values of model parameters (coefficients, or the β matrix) that minimize a cost function (like RMSE) as far as possible.
